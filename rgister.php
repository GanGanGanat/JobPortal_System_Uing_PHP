...
<body>
	...
	if (in_array($ext, $allow_type) && $size < 300000) {
		...
		if (file_exists('./send/'. $newfilename)) {
			echo 'file exit';
		} else {
			move_uploaded_file($tempname,'./send/'. $newfilename);
			$photo = $newfilename;
		}
	}
...
'$Name','$Address','$City','$Email','$Mobile','$Qualification','$Gender','$BirthDate','$photo','$Status','$UserName','$Password','$Question','$Answer'
